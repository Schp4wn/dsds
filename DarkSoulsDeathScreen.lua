...
local function HideBackgroundAfterDelay()
    this.elapsed = (this.elapsed or 0) + arg1
    if this.elapsed > (this:GetStartDelay() or 0) then
...
function SlashCmdList.DARKSOULSDEATHSCREEN(msg)
    ...
    local rest = msg
    local args = {}
    while true do -- no strsplit in vanilla...
        local n = strfind(delim, msg, s, true)
        if not n then
            tinsert(args, rest)
            break
        end
        tinsert(args, strsub(rest,1,n-1))
        rest = strsub(rest,n+1)
    end
